...
**分块结构**：
- Q/K/V：512 × 128
- 第一级分块：只有 1 块（512 × 128）
- 第二级分块：4 个 128 × 128 小块

**单行块计算**：
- QK：Q[1][a] × K[1][1...4] = 4 次矩阵乘
- S 矩阵：128 × 512 × 4B = 256KB
- Softmax：处理 128 × 512
- PV：4 次矩阵乘
...
**Vec 启动延迟**：只等 4 次矩阵乘，而不是 64 次！

---

## 不同序列长度对比

| seq_len | K 块数 | QK 矩阵乘/行 | S 大小 | Vec 等待 | L2 峰值(20核) |
|---------|--------|--------------|--------|----------|---------------|
| 512 | 1 | 4次 | 256KB | 极短 | 5MB |
| 1024 | 2 | 8次 | 512KB | 很短 | 10MB |
| 2048 | 4 | 16次 | 1MB | 短 | 20MB |
| 4096 | 8 | 32次 | 2MB | 中等 | 40MB |
| 8192 | 16 | 64次 | 4MB | 较长 | 80MB |
| 16384 | 32 | 128次 | 8MB | 长 | 160MB |
...
```
seq_len = 512:
Cube:  [QK×4][idle][PV×4][QK×4][idle][PV×4]...
Vec:         [SM]       [SM]       [SM]...
       ↑
       几乎立即启动
```
...
- 单行块 S 矩阵：256KB（vs 8192 的 4MB）
- 20 核峰值：5MB（vs 80MB）
- L2 192MB 完全没有压力

### 3. 同步开销占比更小

Vec 只等 4 次矩阵乘，同步开销占总时间的比例极低。

### 4. 带宽压力分散

每次只写 256KB，不会出现突发带宽竞争。

---

## 短序列下 Online 方案更没必要

| 指标 | 完整行 (512) | Online (512) |
|------|--------------|--------------|
| QK 矩阵乘 | 4次连续 | 4次分散 |
| 同步点 | 2次 | 还是要多次 |
| Rescale | 0次 | 可能3次 |
...
**Online 的唯一优势**（低峰值内存）从 256KB 降到 64KB，**完全没有意义**。

而 Online 的劣势（同步开销、rescale）依然存在。
...
| 场景 | 原因 |
|------|------|
| **短序列（512-2048）** | 流水线快速启动，几乎无等待 |
| **批量推理** | 多个短序列并行，核心利用率高 |
| **编码器模型（BERT 类）** | 典型 seq_len = 512 |
| **固定长度任务** | 分类、匹配、检索等 |
...
1. **流水线启动快**：Vec 等待时间与 seq_len 成正比
2. **L2 压力小**：峰值占用与 seq_len 成正比
3. **同步开销占比低**：总计算量小时，2 次同步几乎可忽略

**短序列 + 完整行方案 = 最佳组合**

而 Online 方案的设计初衷是解决**长序列内存放不下**的问题，在短序列场景下完全是**杀鸡用牛刀**，徒增复杂度和开销。


---

## 本质：完整行方案是 Online 的极端情况

你的完整行方案其实就是**块大小 = seq_len 的 Online softmax**，只不过只有 1 块，所以不需要 rescale。

```
块大小 = 512   → 16 次 rescale
块大小 = 1024  → 8 次 rescale
块大小 = 2048  → 4 次 rescale
块大小 = 4096  → 2 次 rescale（只需 1 次）
块大小 = 8192  → 0 次 rescale（完整行）
```
...
| 方案 | 块大小 | Vec 启动延迟 | Rescale 次数 | 同步次数 | L2 峰值 |
|------|--------|--------------|--------------|----------|---------|
| 细粒度 Online | 512 | 4次矩阵乘 | 15次 | 32次 | 256KB |
| 完整行 | 8192 | 64次矩阵乘 | 0次 | 2次 | 4MB |
...
**时序**：
```
Cube:  [===QK×32===][idle][===PV×32===][===QK×32===][idle][===PV×32===]
Vec:                [SM-1]            [SM-2 + rescale]
```
...
| 块大小 | 块数 | Vec 启动 | Rescale | 同步 | L2 峰值 | 总传输 |
|--------|------|----------|---------|------|---------|--------|
| 512 | 16 | 4次 | 15次 | 32次 | 256KB | 17.1MB |
| 1024 | 8 | 8次 | 7次 | 16次 | 512KB | 14.5MB |
| 2048 | 4 | 16次 | 3次 | 8次 | 1MB | 13.2MB |
| **4096** | **2** | **32次** | **1次** | **4次** | **2MB** | **12.5MB** |
| 8192 | 1 | 64次 | 0次 | 2次 | 4MB | 12.1MB |
...
- 完整行：Vec 等 64 次矩阵乘
- 4096块：Vec 等 32 次矩阵乘（减半）

如果 Cube 很快但 Vec 相对慢，减少启动延迟有意义。

### 2. 只需 1 次 Rescale

```
块1: softmax(S[1:4096]) → O[1]
块2: softmax(S[4097:8192]) → O[2]
最终: rescale(O[1], O[2])
```

1 次 rescale 的开销几乎可以忽略。

### 3. L2 峰值减半

2MB vs 4MB，给其他任务留更多空间。

### 4. 传输量接近最优

12.5MB vs 12.1MB，只多 3%。
...
如果 Vec 处理 softmax 的时间 > Cube 做 32 次矩阵乘的时间：
...
4096块：
Cube:  [==32次==][idle][==32次==][==32次==][idle][==32次==]
Vec:             [中等]        [中等+RS]
```

4096 块可以让 Cube 更早开始第二块的 QK，减少空闲。
...
seq_len = 32768 时：
- 完整行：S = 16MB/行块，20核 = 320MB，超过 L2
- 8192块：S = 4MB/行块，20核 = 80MB，可行
...
```
if seq_len <= L2_capacity / (20 * 4B * 128):
    # 能放下完整行
    if Vec启动延迟不是瓶颈:
        块大小 = seq_len  # 完整行最优
    else:
        块大小 = seq_len / 2  # 减少启动延迟
else:
    # 放不下完整行
    块大小 = 能放下的最大值  # 尽量大，减少 rescale
```
...
**核心原则**：在 L2 容量允许的前提下，**块越大越好**，因为：
1. 减少 rescale 次数
2. 减少同步次数
3. 减少 L2 读写次数

完整行是这个原则的极限，但**不一定是所有场景的最优**。根据具体的：
- 硬件配置（L2 大小、Cube/Vec 速度比）
- 序列长度
- 并发任务

可能存在一个**甜点块大小**（如 4096），在各项指标间取得更好的平衡。